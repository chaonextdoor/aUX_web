...
prevTime:0,finishScrollingObject:null,container:null,maxLeft:0,startLeft:0,rightMargin:0,divWidth:0,touchMove:function(b){try{if(c||(c=!0,this.touchStart(b)),this.currentScrollingObject!=null){b.preventDefault();var a={x:0,y:0},f=0,d=0,l=0,m=0;if(this.verticalScroll){var n=this.lockY-b.touches[0].pageY,n=-n,f=this.startTop+n,d=numOnly((new WebKitCSSMatrix(window.getComputedStyle(this.el).webkitTransform)).f);a.y=f}if(this.horizontalScroll){var o=this.lockX-b.touches[0].pageX,o=-o,n=l=this.startLeft+
...
this.scrollerMoveCSS(this.hscrollBar,{x:p,y:0},0));if(this.prevTime){var r=Math.abs(d)-Math.abs(f),s=Math.abs(m)-Math.abs(l),q=b.timeStamp-this.prevTime;q<1E3?(this.timeMoved+=q,this.vdistanceMoved+=r,this.hdistanceMoved+=s):this.hdistanceMoved=this.vdistanceMoved=this.timeMoved=0}this.prevTime=b.timeStamp}}catch(t){alert("error in scrollMove: "+t)}},touchStart:function(b){var a=this.container,c=this.el;if(a)try{if(b.touches[0].target&&b.touches[0].target.type!=void 0){var f=b.touches[0].target.tagName.toLowerCase();
if(f=="select"||f=="input"||f=="button")return}this.hdistanceMoved=this.vdistanceMoved=this.timeMoved=0;this.finishScrollingObject=this.prevTime=null;this.bottomMargin=a.clientHeight>window.innerHeight?window.innerHeight:a.clientHeight;this.maxTop=c.clientHeight-this.bottomMargin;this.divHeight=c.clientHeight;this.rightMargin=a.clientWidth>window.innerWidth?window.innerWidth:a.clientWidth;this.maxLeft=c.clientWidth-this.rightMargin;this.divWidth=c.clientWidth;if(!(this.maxTop<0)&&b.touches.length==
1&&this.boolScrollLock==!1){try{this.startTop=numOnly((new WebKitCSSMatrix(window.getComputedStyle(c).webkitTransform)).f),this.startLeft=numOnly((new WebKitCSSMatrix(window.getComputedStyle(c).webkitTransform)).e)}catch(d){this.startLeft=this.startTop=0,console.log("error scroller touchstart "+d)}this.lockX=b.touches[0].pageX;this.lockY=b.touches[0].pageY;this.currentScrollingObject=c;if(this.vscrollBar){this.vscrollBar.style.height=parseFloat(this.bottomMargin/this.divHeight)*this.bottomMargin+
