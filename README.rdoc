...
== Mobile Web Libaries (aUX_web.min.js)
...
2. Create your list, broken up by divs/spans/anchors for each alphabet character (A-Z must be capitalized)
...
There are additional functions for aUX.web.scroller

1.  scrollTo

    var scroller = new aUX.web.scroller("my_div");
    scroller.scrollTo({x:0,y:-100}); //Pass in an object with X/Y properties that are negative to scroll to that position.
...
There is a function that allows you to change the password box to "show" or "hide" the value.  The first parameter is 1 or 0 to show/hide the password.  The second is
